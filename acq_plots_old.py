...
def get_source_and_rec_locations_nodes(survey_area_x, survey_area_y, node_dx, node_dy, source_sep, spi, no_sources, shooting_area_overlap, use_dipole_source, no_saillines):
    ...
    if no_sources==1:
        source_sep=source_sep/2 #to get this right
    ...
    Sx = np.linspace(-shooting_area_overlap, survey_area_x+shooting_area_overlap, 1+round((survey_area_x+2*shooting_area_overlap)/(spi)))
    ...
    #New: if we have one dipole source:   1 x x 4 x x 7 x x 10
    #New: if we have tw0 dipole sources:  1 2 x x 5 6 x x 9 10 x #
    #New: if we have three dipole sources 1 2 3 x x 6 7 8 x x 11 12 13 x x
    ...
    while (count2<len(Sy)):
        for i in range(0,no_sources):  #lines to be shot
            if(count2<len(Sy)):
                ...
                count2=count2+1
        for i in range(0,2): #lines to be interpolated
            if((count2<len(Sy)) and (use_dipole_source==True)):
                source_lines[count2]=0
                count2=count2+1

    #fill in the source lines that are swept
...
def plot_node_survey_layout(R, S, S_dipole, no_sources, source_sep):

    if no_sources==1:
        source_sep = source_sep/2 #to get the one source thisng right

    my_size=1
    if (sum(R[:,0].shape) <100):  #to scale the size of the markers
        my_size=3
    elif (sum(R[:,0].shape) <300):  #to scale the size of the markers
        my_size=3

    #plotting both without and with depole source
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=R[:,0], y=R[:,1], mode='markers', name='node', marker_size=3*my_size ))
    fig.add_trace(go.Scatter(x=S[:,0], y=S[:,1], mode='markers', name='source point', marker_size=1*my_size))
    survey_area = (max(R[:,0])-min(R[:,0])) * (max(R[:,1])-min(R[:,1]))/1000000  #in km2
    no_sp = sum(S[:,0].shape)
    no_nodes =sum(R[:,0].shape)
    txt = ('Node survey of %d km2 with %d nodes and %d shotpoints.<br>%d sources@%2.1fm separation.' %(survey_area, no_nodes, no_sp,no_sources, source_sep))
    fig.update_layout(title=txt,  xaxis_title='Meter',yaxis_title='Meter')

    fig1 = go.Figure()
    fig1.add_trace(go.Scatter(x=R[:,0], y=R[:,1], mode='markers', name='node', marker_size=3*my_size ))
    fig1.add_trace(go.Scatter(x=S_dipole[:,0], y=S_dipole[:,1], mode='markers', name='dipole point', marker_size=1*my_size))
    survey_area = (max(R[:,0])-min(R[:,0])) * (max(R[:,1])-min(R[:,1]))/1000000  #in km2
    no_sp = sum(S_dipole[:,0].shape)
    no_nodes =sum(R[:,0].shape)
    txt = ('Dipole node survey of %d km2 with %d nodes and %d shotpoints.<br>%d sources@%2.1fm separation + 2 virtual source lines between each sail-line.' %(survey_area, no_nodes, no_sp,no_sources, source_sep))
    #find the source-line spacing:
    # sl_spacing1 = max(np.diff(S_dipole[:,1]))
    # sl_spacing2 = min(abs(np.diff(S_dipole[:,1])))
    # print("sl spacing:",sl_spacing1 ,sl_spacing2 )
    # if (sl_spacing1 != sl_spacing2):
    #     txt = txt+"<br>The source separation was " +str(round(100*sl_spacing2)/100)+"m with 2 virtual source lines between each sailline."
    # else:
    #     txt = txt+"<br>The source-line seraration was "+str(sl_spacing1)+"m."
...
def get_source_and_streamer_layout(no_lf_sources, source_lf_sep, no_hf_sources, source_hf_sep, no_streamers, streamer_sep, streamer_sep2, streamer_len, layback, place_lf_in_center):

    #first to the streamer layout: x=inline, y=crossline
    streamer_y = np.linspace(0, streamer_sep*(no_streamers-1), no_streamers) #This is the nominal plot_layout_
    if streamer_sep2[0] != 0:                                            #then we know that the user spesified this
        streamer_y=np.zeros(no_streamers)
        for i in range(0,no_streamers-1):
            streamer_y[i+1]=streamer_y[i]+streamer_sep2[i]                                 #set the new streamer sep
    ...
    if streamer_sep2[0] !=0:                 #WORKING HERE June 2021
        mid = 0.5*np.sum(streamer_sep2)
...
#Compute the source and streamer layout - and return the appropriate arrays
def get_source_and_streamer_layout_short(no_sources, source_sep, no_streamers, streamer_sep,  streamer_sep2, streamer_len, layback):


    streamer_x = np.linspace(0, streamer_sep*(no_streamers-1), no_streamers)

    if streamer_sep2[0] != 0:                                            #then we know that the user spesified this
        streamer_x=np.zeros(no_streamers)
        for i in range(0,no_streamers-1):
            streamer_x[i+1]=streamer_x[i]+streamer_sep2[i]                                 #set the new streamer sep
...
def plot_compare_streamer_and_node_offset(no_sources, source_sep, spi, no_streamers, streamer_sep, streamer_sep2, streamer_len, layback, R, S, S_dipole, shooting_area_overlap ):

    [source_x, source_y, streamer_x, streamer_y] = get_source_and_streamer_layout_short(no_sources, source_sep, no_streamers, streamer_sep, streamer_sep2, streamer_len, layback)
...
def plot_layout_birdseye(streamer_sep, source_x,  source_y, streamer_x, streamer_y, hf_source, no_source_vessels, source_vessel_offset_crossline, source_vessel_offset_inline, no_hf_sources):
    fig = go.Figure()
    no_streamers = len(streamer_x)
    no_sources = len(source_x)
    source_sep = 0.0
...
def plot_streamer_survey_offset_distribution_from_shot(offsets, no_sources, spi, no_streamers, streamer_sep, streamer_sep2, streamer_len, layback):
    fig = go.Figure()
...
def plot_energy(no_sources, source_sep, spi, no_streamers, streamer_sep, streamer_sep2,streamer_len, layback, R, S, S_dipole, shooting_area_overlap, critical_reflection_angle):

    [source_x, source_y, streamer_x, streamer_y] = get_source_and_streamer_layout_short(no_sources, source_sep, no_streamers, streamer_sep, streamer_sep2,  streamer_sep2, streamer_len, layback)
    #get the offsets from the node-survey
    [offsets_streamer, cmp_x_streamer, cmp_y_streamer]=get_offsets_from_one_shot_on_a_spread(source_x, source_y, streamer_x, streamer_y)
    #get the offsets from the streamer survey
    [offsets_node, cmp_x_node, cmp_y_node, node_dx, node_dy, node_area_x, node_area_y, survey_area_x, survey_area_y] = get_offset_from_one_shot_on_nodes(R, S, shooting_area_overlap)
...
def acq_plots():
    ...
    with my_expander1:
        ...
        with col2:
            ...
            if len(uneven_streamer_sep)>0:
                uneven_streamer_sep =uneven_streamer_sep.split(",")
                if len(uneven_streamer_sep)==(no_streamers-1):
                    for i in range(0,len(uneven_streamer_sep)):
    ...
    with my_expander0:
        ...
        [source_lf_x, source_lf_y, source_hf_x, source_hf_y, streamer_x, streamer_y] = get_source_and_streamer_layout(no_lf_sources, source_lf_sep, no_hf_sources, source_hf_sep, no_streamers, streamer_sep,  streamer_sep2, streamer_len, layback, place_lf_in_center)


        if extra_source_vessel_1: #this allows for example side-by-side or TopSeis mode
            [source_vx, source_vy, tmp1,tmp2 ] = get_source_and_streamer_layout_short(source_vessel_no_sources, source_vessel_source_sep, no_streamers, streamer_sep, streamer_sep2, streamer_len, layback)
    ...
    with my_expander3:
        ...
        with col2:
            survey_area_x = st.number_input('Survey area x (km)',1,100,1,1)
            survey_area_y = st.number_input('Survey area y (km)',1,100,2,1)
        ...
        with col3:
            shooting_area_overlap = st.number_input('Shooting area outside nodes (km)',0,10,0,1)
        ...
        if (plot_node_layout):
            ...
            [fig, fig1] = plot_node_survey_layout(R, S, S_dipole, no_hf_sources, source_hf_sep)
        if (plot_offset_rec):
            [R, S] = get_source_and_rec_locations_nodes(survey_area_x, survey_area_y, node_dx, node_dy, source_hf_sep, spi,no_hf_sources, shooting_area_overlap, False, no_saillines)
            [R, S_dipole] = get_source_and_rec_locations_nodes(survey_area_x, survey_area_y, node_dx, node_dy, source_hf_sep, spi,no_hf_sources, shooting_area_overlap, True, no_saillines)
        ...
        if (compare_offset_node_str):
            [R, S] = get_source_and_rec_locations_nodes(survey_area_x, survey_area_y, node_dx, node_dy, source_hf_sep, spi,no_hf_sources, shooting_area_overlap, False, no_saillines)
            [R, S_dipole] = get_source_and_rec_locations_nodes(survey_area_x, survey_area_y, node_dx, node_dy, source_hf_sep, spi,no_hf_sources, shooting_area_overlap, True, no_saillines)
            fig = plot_compare_streamer_and_node_offset(no_hf_sources, source_hf_sep, spi, no_streamers, streamer_sep, streamer_sep2, streamer_len, layback, R, S, S_dipole, shooting_area_overlap )
        ...
        if (plot_offset_shot_streamer):
            [source_x, source_y, streamer_x, streamer_y] = get_source_and_streamer_layout_short(no_hf_sources, source_hf_sep, no_streamers, streamer_sep, streamer_sep2, streamer_len, layback)
            [offsets, cmp_x, cmp_y]=get_offsets_from_one_shot_on_a_spread(source_x, source_y, streamer_x, streamer_y)
            fig = plot_streamer_survey_offset_distribution_from_shot(offsets, no_hf_sources, spi, no_streamers, streamer_sep, streamer_sep2, streamer_len, layback)
    ...
    with my_expander2:
        ...
        if(plot_hf_birdseye):
            ...
            fig =  plot_layout_birdseye(streamer_sep, source_y, source_x, streamer_y, streamer_x, hf_source, no_source_vessels, source_vessel_offset_crossline, source_vessel_offset_inline, no_hf_sources)
