...
def get_source_y(no_sources, mid, source_sep):
    ...
    if(no_sources==1):
        ...
        st.write('Only up to 6 sources is supported for now!')
...
def get_source_and_streamer_layout(no_lf_sources, source_lf_sep, no_hf_sources, source_hf_sep, no_streamers, streamer_sep, streamer_len, layback, place_lf_in_center):
    ...
    if place_lf_in_center==True: #we place the lf-soures in the center - which is good both geophysically and practically (they are deep)
        ...
        source_lf_x=layback*np.ones((len(source_lf_y)))
        source_hf_x=layback*np.ones((len(source_hf_y)))
...
def plot_compare_streamer_and_node_offset(no_sources, source_sep, spi, no_streamers, streamer_sep, streamer_len, layback, R, S, S_dipole, shooting_area_overlap ):
    ...
    fig.update_layout(
        title_text='Single shot (middle of the spread) offset distribution on:<br>'+str(no_streamers)+' streamers @ '+str(streamer_sep)+'m and<br>length '+str(streamer_len)+' m. The source layback was '+str(layback)+'m. '+txt+'<br>'+str(len(R))+' nodes with spacing (dx, dy)=('+str(round(node_dx))+', '+str(round(node_dy))+')m. The node area: '+str(round(node_area_x/1000))+' X '+str(round(node_area_x/1000))+' km.',
        xaxis_title_text='Offset groups of '+str(round(2*spi*no_sources))+' m.', # xaxis label
        yaxis_title_text='TraceCount', # yaxis label
        bargap=0.1, # gap between bars of adjacent location coordinates
        )
...
def plot_offset_classes(streamer_sep, source_x, source_y, streamer_x, streamer_y, offset_class_size):
    fig = go.Figure()
    ...
    for i in range(0, no_sources):
        ...
        txt='Offset distribution for '+str(no_streamers)+' streamers @'+str(streamer_sep)+'m and '+str(no_sources)+' HF sources @'+str(source_sep)+'m.<br>The offset class size is '+str(offset_class_size)+'m.'
        fig.update_layout(title=txt, xaxis_title='Meter',yaxis_title='CMP offset class (Meter)', showlegend=False)

    return fig

#plot the layout of the source and streamer setup - birdseye-view
def plot_layout_birdseye(streamer_sep, source_x, streamer_x, streamer_y, hf_source ):
    fig = go.Figure()
    no_streamers = len(streamer_x)
    no_sources = len(source_x)
    source_y = np.zeros((no_sources))
    ...
    cmp_y.append(0.5*(source_y[0]+streamer_y[0]))   #first point (source + front streamer /2)
    cmp_y.append(0.5*(source_y[0]+streamer_y[-1]))   #last point  (source + back streamer) /2
    ...
    my_colors2 = ['rgba(255,0,0,0.1)', 'rgba(0,255,0,0.1)', 'rgba(0,0,255,0.1)', 'rgba(128,0,128,0.1)','rgba(0,255,255,0.1)', 'rgba(255,127,80,0.1)', 'rgba(0,0,0,0.1)' ]
    my_colors02 = []
    ...
    for i in range(0, len(cmp_x)): #the cmp's (Need to go first due to the filling option)
        fig.add_trace(go.Scatter(x=[cmp_x[i], cmp_x[i]], y=[cmp_y[0], cmp_y[1]], fill='tonexty', mode='lines', line_width=0.2, line_color=my_colors[i], fillcolor=my_colors2[i] ))
    ...
    fig.add_trace(go.Scatter(x=source_x, y=np.zeros((no_sources)), mode='markers', name='sources', marker_size=my_marker_size, marker_symbol='star', marker=dict(color='black', line=dict(color=my_colors, width=1))  ))
    ...
    #draw a vessel
    fig.add_trace(go.Scatter(x=[mid-12, mid+12], y=[-300, -300],  line=dict(color='black', width=3), mode='lines' ))
    fig.add_trace(go.Scatter(x=[mid-12, mid-12], y=[-300, -370],  line=dict(color='black', width=3), mode='lines' ))
    fig.add_trace(go.Scatter(x=[mid+12, mid+12], y=[-300, -370],  line=dict(color='black', width=3), mode='lines' ))
    fig.add_trace(go.Scatter(x=[mid-12, mid],    y=[-370, -400],  line=dict(color='black', width=3), mode='lines' ))
    fig.add_trace(go.Scatter(x=[mid+12, mid],    y=[-370, -400],  line=dict(color='black', width=3), mode='lines' ))

    #draw a spline from the vessel to the sources
    sorce_x= source_x.sort()
    ...
    if streamer_y[0]>0: #Normal mode
        ...
        fig.add_trace(go.Scatter(x=[mid-12, mid+12], y=[-300+streamer_y[0], -300+streamer_y[0]],  line=dict(color='black', width=3), mode='lines' ))
        fig.add_trace(go.Scatter(x=[mid-12, mid-12], y=[-300+streamer_y[0], -370+streamer_y[0]],  line=dict(color='black', width=3), mode='lines' ))
        fig.add_trace(go.Scatter(x=[mid+12, mid+12], y=[-300+streamer_y[0], -370+streamer_y[0]],  line=dict(color='black', width=3), mode='lines' ))
        fig.add_trace(go.Scatter(x=[mid-12, mid],    y=[-370+streamer_y[0], -400+streamer_y[0]],  line=dict(color='black', width=3), mode='lines' ))
        fig.add_trace(go.Scatter(x=[mid+12, mid],    y=[-370+streamer_y[0], -400+streamer_y[0]],  line=dict(color='black', width=3), mode='lines' ))
    ...
    fig.update_yaxes(range=[-450, 500])
    fig.update_xaxes(range=[streamer_x[0]-50, streamer_x[-1]+50])


    return fig

#plot the source and streamer layout, inline
def plot_layout_(streamer_sep, source_x, source_y, streamer_x, streamer_y, text, fig=0, shift=0.0, make_thick=1):
    ...
    if no_sources>0:
        dy = 0.5*streamer_sep/no_sources
        y1=-0.8
    ...
    if no_sources>0:
        txt = ('Inline view: ('+text+')<br>'+str(no_streamers)+' str @ '+str(streamer_sep)+' m and '+str(no_sources)+' src @'+str(source_sep)+' m. The natural bin size (dy)= '+str(0.01*round(100*0.5*streamer_sep/no_sources))+' m.')
    else:
        txt = ('Inline view: ('+text+')<br>'+str(no_streamers)+' str @ '+str(streamer_sep)+' m and '+str(no_sources)+' src @'+str(source_sep)+' m. Effective CMP-width is '+str(0.5*no_streamers*streamer_sep)+' m.')
    ...
    shift_x = [0, 0.5*no_streamers*streamer_sep, 1.0*no_streamers*streamer_sep, 1.5*no_streamers*streamer_sep]
...
def plot_combined_layout_(streamer_sep, source_lf_x, source_lf_y, source_hf_x, source_hf_y, streamer_x, streamer_y):

    #first we plot the HF source - returning a fig
    fig = go.Figure()
    [fig, fig1] = plot_layout_(streamer_sep, source_hf_x, source_hf_y, streamer_x, streamer_y,  "HF source layout")
    [fig, fig1] = plot_layout_(streamer_sep, source_lf_x, source_lf_y, streamer_x, streamer_y,  "LF source layout", fig, shift=-0.05, make_thick=1.8)
    source_sep_hf = 0.0
    ...
    if len(source_hf_x) >0 and len(source_lf_x)>0:
        txt = ('Inline view: <b>LF</b> and HF vibes:<br>'+str(no_streamers)+' str @ '+str(streamer_sep)+'m. '+str(len(source_hf_x))+' HF vib groups @'+str(round(source_sep_hf*100)/100)+'m and '+str(len(source_lf_x))+' LF vib groups @'+str(round(source_sep_lf*100)/100)+'m. <br>X-line bin sizes (HF, LF)=('+str(0.5*streamer_sep/len(source_hf_x))+', '+str(0.5*streamer_sep/len(source_lf_x))+')m. Sailline width='+str(0.5*no_streamers*streamer_sep)+'m.')
    else:
...
def make_rose_plot_streamer(streamer_x, streamer_y, source_x, r_alfa, sector_len):
    ...
    theta = np.linspace(0,2*np.pi, round(360/r_alfa))  #start, stop, numb: 6 deg sectors-->60 numb
    ...
    for i in range(0,len(source_x)):
        for j in range(0,len(streamer_y)):
            ...
            for k in range(0, len(streamer_x)):
                ...
                t_index = binarySearch(theta,  math.atan2(tmp1,tmp0 ) + math.pi)
                C[r_index, t_index] = C[r_index, t_index] +1
                count = count + 1
    ...
    df = pd.DataFrame({'R':R, 'T':T, 'TraceCount':TraceCount}) #make a df
    fig = px.bar_polar(df, r="R", theta="T", color="TraceCount",   color_continuous_scale=px.colors.sequential.Jet, title='Roseplot from single vessel: '+str(len(source_x))+' sources@'+str((source_x[min(1,len(source_x)-1)]-source_x[0]))+'m and '+str(len(streamer_x))+' streamers@'+str(round(streamer_y[-1]-layback))+'m.<br>The source layback was '+str(round(layback))+'m.')
...
def make_rose_plot_nodes(Receiver, S, no_sources, r_alfa, sector_len,  node_dx, node_dy, spi):
    ...
    df = pd.DataFrame({'R':R, 'T':T, 'TraceCount':TraceCount}) #make a df
    fig = px.bar_polar(df, r="R", theta="T", color="TraceCount",   color_continuous_scale=px.colors.sequential.Jet,title='Roseplot from '+str(no_sources)+' sources@'+str(source_offset)+
...
def main():
    print("Hello World!")
    st.sidebar.subheader('Version 0.1.0 - March 2021')
    st.sidebar.write('Welcome to the Shearwater reflection marine survey assesment tool. This python based web-site allows the user to quickly produce high quality plots of source, streamer and node layouts. This should be useful to make illustrations for reports, scientific papers and presentations.')
    st.sidebar.write('The code is still under development, and there might still be some bugs. On the source side, conceptually, a HF, LF or air-gun source will all be plotted as a star. ')
    st.sidebar.write('If you have questions, comments or features you would like to see/add, please contact Thomas Elboth: telboth@shearwatergeo.com')

    st.image('Shearwater_logo400x85.png')
    ...
    # Lets make columns with widgets in
        ...
        with col2:
            place_lf_in_center = st.checkbox('Place the LF source(s) in the center with the HF on the sides', value=True)

        with col2:
            no_streamers = st.selectbox('Number of streamers' ,[12,0,6,8,10,12,14,16,18,20])
            streamer_sep = st.selectbox('Streamer sep (m)',[100, 50, 62.5, 75, 83.33, 90, 100, 110, 120, 150, 200])
            layback = st.selectbox('Source layback (m)',[150, 100, 75, 50, 0, -3000])


        with col3:
            if place_lf_in_center:
                no_hf_sources= st.number_input('Number of HF source groups',0,8,2,2)
                no_lf_sources = st.number_input('Number of LF source groups',1,8,2,1)
            else:
                no_hf_sources= st.number_input('Number of HF source groups',1,8,2,1)
                no_lf_sources = st.number_input('Number of LF source groups',0,8,2,2)
            ...
            source_hf_sep = st.selectbox('HF source group sep (m)', ss)

        with col1:
            plot_hf_layout = st.button("Plot inline HF layout")
    ...
    with my_expander3:
        ...
        sail_line_sep = no_hf_sources*(streamer_sep/no_hf_sources)  #Check
    ...
    with my_expander2:
        ...
        if (plot_hf_layout):
            [fig, fig1] = plot_layout_(streamer_sep, source_hf_y, source_hf_x, streamer_y, streamer_x,  "HF source layout")

        if (plot_lf_layout):
            [fig, fig1] = plot_layout_(streamer_sep, source_lf_y, source_lf_x, streamer_y, streamer_x,  "LF source layout")

        if(plot_tot_layout):
            fig = plot_combined_layout_(streamer_sep, source_lf_y, source_lf_x, source_hf_y, source_hf_x, streamer_y, streamer_x)
        ...
        if(plot_hf_birdseye):
            ...
            fig =  plot_layout_birdseye(streamer_sep, source_y, streamer_y, streamer_x, hf_source)
            if(no_hf_sources>0 and no_lf_sources>0):
                txt = ('Birdseye view (front end): '+str(no_streamers)+' str @ '+str(streamer_sep)+'m. <br>'+str(no_hf_sources)+' <b>HF</b> src @'+str(source_hf_sep)+'m and '+str(no_lf_sources)+' LF src @'+str(source_lf_sep)+'m. Effective CMP-width is '+str(0.5*no_streamers*streamer_sep)+' m.<br>The X-line bin sizes (HF, LF) is ('+str(0.5*streamer_sep/no_hf_sources)+', '+str(0.5*streamer_sep/no_lf_sources)+')m.')
            else:
                txt = ('Birdseye view (front end): '+str(no_streamers)+' str @ '+str(streamer_sep)+'m. <br>'+str(no_hf_sources)+' <b>HF</b> src @'+str(source_hf_sep)+'m and '+str(no_lf_sources)+' LF src @'+str(source_lf_sep)+'m. Effective CMP-width is '+str(0.5*no_streamers*streamer_sep)+' m.')
            fig.update_layout(title=txt, xaxis_title='Meter',yaxis_title='Meter', showlegend=False)
            if no_hf_sources>0 and no_streamers>0:
                fig1 = plot_offset_classes(streamer_sep, source_hf_y, source_hf_x, streamer_y, streamer_x, offset_class_size)
                fig1.update_xaxes(range=[streamer_y[0]-50, streamer_y[-1]+50])

        if make_roseplot:
            fig = make_rose_plot_streamer(streamer_y, streamer_x, source_hf_y, r_alfa, sector_len)
            fig1 = make_rose_plot_streamer(streamer_y, streamer_x, source_lf_y, r_alfa, sector_len)
